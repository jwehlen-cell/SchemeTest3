...
COMMENT ON TABLE amplitude_dyn_pars_dbl IS $$ The amplitude_dyn_pars_dbl table will store amplitude parame ters and metadata (floatvalues). Each record will contain o nly one piece of information. $$;
...
COMMENT ON TABLE amplitude_dyn_pars_int IS $$ The amplitude_dyn_pars_int table will store amplitude parame ters and metadata (integervalues). Each record will contain only one piece of information. $$;
...
COMMENT ON TABLE amplitude_dyn_pars_str IS $$ The amplitude_dyn_pars_str table will store amplitude parame ters and metadata (stringvalues). Each record will contain only one piece of information. $$;
...
COMMENT ON TABLE arrival_dyn_pars_dbl IS $$ The arrival_dyn_pars_dbl table will store arrival parameters and metadata (floatvalues). Each record will contain only one piece of information. $$;
...
COMMENT ON TABLE arrival_dyn_pars_int IS $$ The arrival_dyn_pars_int table will store arrival parameters and metadata (integervalues). Each record will contain onl y one piece of information. $$;
...
COMMENT ON TABLE arrival_dyn_pars_str IS $$ The arrival_dyn_pars_str table will store arrival parameters and metadata (stringvalues). Each record will contain only one piece of information. $$;
...
COMMENT ON COLUMN classification.class_opr_id IS $$ ID for the classification_opr table. This is the agency, office (orOPR) responsible for classification and classification date. $$;
...
COMMENT ON TABLE classification_opr IS $$ Used to contain the agency, office (orOPR) responsible for classification $$;
...
COMMENT ON TABLE cluster_info IS $$ The cluster_info table contains information relating to the cluster of events. It relates to versions of the same cluste r (sameevents) together. It also contains the orid of the e vent closest to the center of the cluster and the radius of the cluster. $$;
...
COMMENT ON COLUMN codamag_input.ffactor IS $$ Yield F factor (log10) $$;
...
COMMENT ON TABLE datauser IS $$ The datauser table tracks authorized users of the Message an d Subscription Subsystems. Each user is identified by a (unique) username and domain, which must match all e-mail header s. The priority column specifies the class of user, and serv icetime is the last time a request from the user was process ed. Priority and servicetime are considered when selecting t he order in which requests will be processed. The status can either be active or inactive. $$;
...
COMMENT ON COLUMN datauser.emaillimit IS $$ Maximum size of message (inbytes) that will be delivered via e-mail $$;
...
COMMENT ON TABLE filter_values_double IS $$ The filter_values_double table will store filter parameters and metadata (floatvalues). Each record will contain only o ne piece of information. Each filter type will store only th ose pieces of information that are pertinent to that filter type. A butterworth filter will have fhi and flo attribute r ecords while digital filters may have num_numerator_coeffs o r compute_time_shift attributes. $$;
...
COMMENT ON TABLE filter_values_integer IS $$ The filter_values_double table will store filter parameters and metadata (integervalues). Each record will contain only one piece of information. Each filter type will store only those pieces of information that are pertinent to that filte r type. A butterworth filter will have fhi and flo attribute records while digital filters may have num_numerator_coeffs or compute_time_shift attributes. $$;
...
COMMENT ON TABLE filter_values_string IS $$ The filter_values_string table will store filter parameters and metadata (stringvalues). Each record will contain only one piece of information. Each filter type will store only those pieces of information that are pertinent to that filte r type. A butterworth filter will have fhi and flo attribute records while digital filters may have num_numerator_coeffs or compute_time_shift attributes. $$;
...
COMMENT ON COLUMN hydro_arr_group.az2 IS $$ Second possible azimuth (2arrivals) $$;
...
COMMENT ON TABLE instrument IS $$ The instrument table contains ancillary calibration informat ion. It holds nominal one-frequency calibration factors for each instrument and pointers to nominal frequency-dependent calibration for an instrument. This table also holds pointer s to the exact calibrations obtained by direct measurement o n a particular instrument (seesensor). $$;
...
COMMENT ON TABLE network IS $$ The network table contains general information about seismic networks (seeaffiliation). $$;
...
COMMENT ON COLUMN netyield.ffactor IS $$ Yield F factor (log10) $$;
...
COMMENT ON COLUMN request.modtime IS $$ Time of last state change (epochtime) $$;
...
COMMENT ON COLUMN stayield.ffactor IS $$ Yield F factor (log10) $$;
...
COMMENT ON TABLE svddisc IS $$ The svddisc table contains a left singular vector (LSV) head er file and descriptive information. This table provides a p ointer (orindex) to singular value decompositions stored on disk. The LSVs are stored in ordinary disk files called svd disc or '.lsv' files as a sequence of binary four byte float values. $$;
...
COMMENT ON COLUMN svddisc.sta IS $$ Station code (RefSta) $$;
...
COMMENT ON COLUMN tomo_info.dlat IS $$ Latitude grid spacing (latitudeinterval) $$;
COMMENT ON COLUMN tomo_info.minlon IS $$ Minimum longitude $$;
COMMENT ON COLUMN tomo_info.maxlon IS $$ Maximum longitude $$;
COMMENT ON COLUMN tomo_info.dlon IS $$ Longitude grid spacing (longitudeinterval) $$;
...
COMMENT ON COLUMN user_privileges.auth IS $$ Author (authorizeduser) $$;
...
COMMENT ON TABLE wfdisc IS $$ The wfdisc table contains a waveform header file and descrip tive information. This table provides a pointer (orindex) t o waveforms stored on disk. The waveforms themselves are sto red in ordinary disk files called wfdisc or '.w' files as a sequence of sample values (usually in binary representation) . $$;
...
ALTER TABLE cluster_magref ADD CONSTRAINT  FOREIGN KEY (activetime) REFERENCES cluster_station (activetime);
ALTER TABLE cluster_mec ADD CONSTRAINT clu_mec_clu_sta_ca_at_fk FOREIGN KEY (clustaid) REFERENCES cluster_station (clustaid);
ALTER TABLE cluster_mec ADD CONSTRAINT  FOREIGN KEY (activetime) REFERENCES cluster_station (activetime);
...
ALTER TABLE cluster_orid ADD CONSTRAINT  FOREIGN KEY (activetime) REFERENCES cluster_station (activetime);
...
ALTER TABLE cluster_override ADD CONSTRAINT  FOREIGN KEY (activetime) REFERENCES cluster_station (activetime);
...
ALTER TABLE cluster_threshold ADD CONSTRAINT  FOREIGN KEY (activetime) REFERENCES cluster_station (activetime);
...
ALTER TABLE station_processing ADD CONSTRAINT  FOREIGN KEY (activetime) REFERENCES cluster_station (activetime);
...
ALTER TABLE svddisc_chan_order ADD CONSTRAINT  FOREIGN KEY (activetime) REFERENCES cluster_station (activetime);
